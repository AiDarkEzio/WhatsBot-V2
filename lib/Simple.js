...
class WAConnection {
    ...
    if (options.asSticker || /webp/.test(mime)) {
      ...
      pathFile = await writeExif(media, {
        packname: options.packname
          ? options.packname
          : ezio.config.exif.packname,
        author: options.author ? options.author : ezio.config.exif.author,
  ...
  async sendErrorMessage(jid, error, key, quoted, btn = [], onReat = true) {
    ...
    await this.sendMessage(
      jid,
      {
        image: { url: ezio.config.image.url.D_E_ERR },
        footer: `${ezio.config.exif.footer} Error Message`,
        buttons: btn,
        caption: ezio.errorMessage(error),
      },
      { quoted: quoted }
    );
  }
