...
const make_ordered_dictionary_1 = require("Wa-Web/lib/Store/make-ordered-dictionary");
const waChatKey = (pin) => ({
  key: (c) =>
    (pin ? (c.pin ? "1" : "0") : "") +
    (c.archive ? "0" : "1") +
...
const makeMessagesDictionary = () =>
  make_ordered_dictionary_1.default(waMessageID);
...
function bind(conn) {
  ...
  conn.ev.on("chats.set", ({ chats: newChats, isLatest }) => {
    /**if (isLatest) {
            chats.clear()
        }**/
  ...
  conn.ev.on("messages.set", ({ messages: newMessages, isLatest }) => {
    if (isLatest) {
  ...
  return {
    ...
    loadMessages: async (jid, count, cursor) => {
      ...
      const retrieve = async (count, cursor) => {
        const result = await (conn === null || conn === void 0
          ? void 0
          : conn.fetchMessagesFromWA(jid, count, cursor));
        return result || [];
      };
      ...
      if (list && mode === "before" && (!cursorKey || cursorValue)) {
        ...
        if (diff < 0) {
          ...
          const cursor = {
            before:
              (fMessage === null || fMessage === void 0
                ? void 0
                : fMessage.key) || cursorKey,
          };
    ...
    loadMessage: async (jid, id) => {
      var _a;
      let message =
        (_a = messages[jid]) === null || _a === void 0 ? void 0 : _a.get(id);
      if (!message) {
        message = await (conn === null || conn === void 0
          ? void 0
          : conn.loadMessageFromWA(jid, id));
      }
      return message;
    },
    mostRecentMessage: async (jid) => {
      var _a;
      let message =
        (_a = messages[jid]) === null || _a === void 0
          ? void 0
          : _a.array.slice(-1)[0];
      if (!message) {
        const [result] = await (conn === null || conn === void 0
          ? void 0
          : conn.fetchMessagesFromWA(jid, 1, undefined));
        message = result;
      }
      return message;
    },
    fetchImageUrl: async (jid) => {
      ...
      if (!contact) {
        return conn === null || conn === void 0
          ? void 0
          : conn.profilePictureUrl(jid);
      }
      if (typeof contact.imgUrl === "undefined") {
        contact.imgUrl = await (conn === null || conn === void 0
          ? void 0
          : conn.profilePictureUrl(jid));
      }
    ...
    fetchGroupMetadata: async (jid) => {
      if (!groupMetadata[jid]) {
        groupMetadata[jid] = await (conn === null || conn === void 0
          ? void 0
    ...
    fetchMessageReceipts: async ({ remoteJid, id }) => {
      ...
      if (!receipts) {
        receipts = await (conn === null || conn === void 0
          ? void 0
          : conn.messageInfo(remoteJid, id));
        if (msg) {
